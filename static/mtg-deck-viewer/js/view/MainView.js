class MainView
{
    constructor(magicController)
    {
        ...
        this.dom = $$(div({style: 'height: 100%; width: 100%'}),
            ...
            this.side = div({style: 'position: absolute; right: 0px; width: 50px; background-color: blue;'})
        );
    ...
    refresh(deckModel)
    {
        ...
        if (deckModel.sideboard.length != 0) {
          creatures.push([]);
          noncreatures.push([]);
          let side_len = Math.max(creatures.length, noncreatures.length);
          for (let x = 0; x < side_len; x++) {
            sideboard.push([]);
          }
          sideboard.push(deckModel.sideboard);
          creatures.push([]);
          noncreatures.push([]);
          this.calculate_scale(this.dom, creatures, noncreatures, deckModel.lands);
          this.redraw(deckModel, creatures, noncreatures);
          this.draw_group(this.crit, sideboard);
        } else {
          creatures.push([]);
          noncreatures.push([]);
          this.calculate_scale(this.dom, creatures, noncreatures, deckModel.lands);
    ...
    draw_group(target, listgroup) {
        for (let x = 0; x < listgroup.length; x++) {
            for (let y = 0; y < listgroup[x].length; y++) {
                let cardjson = listgroup[x][y];
                this.draw(target, x, y, cardjson);
            }
        }
    }

    draw(target,x,y,cardjson)
    {
