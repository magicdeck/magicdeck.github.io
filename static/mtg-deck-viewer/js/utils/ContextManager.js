...
class ContextManager {
    ...
    buildDom() {
        let dom = $$(div({style: 'width: 100%; text-align: center;'}),
            ...
            $$(table({style: 'width: 100%;'}),
              $$(tr({style: 'width: 100%;'}),
                ...
                $$(td({style: 'width: 15%;'}),
                  $$(this.loadButton = $$$(button({style: 'display: block; width: 100%; padding: 0;'}), 'load'))
                ),
                $$(td({style: 'width: 15%;'}),
                  $$(this.deleteButton = $$$(button({style: 'display: block; width: 100%; padding: 0;'}), 'delete'))
                )
              ),
              $$(tr(),
                $$(td({style: 'width: 70%;'}),
                  this.newLabel = input({style: 'display: block; width: 95%; padding: 0; margin: 0;', type: 'text'})
                ),
                $$(td({style: 'width: 15%;'}),
                  $$(this.saveasButton = $$$(button({style: 'display: block; width: 100%; padding: 0;'}), 'store'))
                ),
                $$(td({style: 'width: 15%;'}),
                  $$(this.renameButton = $$$(button({style: 'display: block; width: 100%; padding: 0;'}), 'rename'))
                )
              )
            )
    ...
    restoreContext() {
        if (typeof(Storage) !== "undefined") {
            if (localStorage.contextCollection) {
                ...
                this.contextCollection = defaultContextCollection;
                this.selectedKey = "Final Adventure";
                this.context = copy(this.contextCollection[this.selectedKey]);
            }
        }

        if (typeof(API_CONTEXT) !== 'undefined') {
            let KEY = API_CONTEXT['name']
            this.contextCollection[KEY] = copy(API_CONTEXT);
            this.context = copy(API_CONTEXT);
            this.selectedKey = KEY;
        }
